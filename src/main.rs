//! # semantic-search - Punto de entrada de la aplicación

use clap::Parser;
use colored::*;
use dirs;
use semantic_search::cli::{Cli, Commands, AliasAction};
use semantic_search::core::{index_files, search_files, SearchConfig};
...
pub fn get_config_path() -> std::path::PathBuf {
    if let Some(config_dir) = dirs::config_dir() {
        ...
        std::path::PathBuf::from(".semantic-search-config.toml")
    }
}
...
fn apply_config_to_search(
    ...
    config: &Config,
) -> (Option<Vec<String>>, Vec<String>, bool, bool, Option<String>, Option<String>) {
    ...
    (ext_vec, ignore_vec, new_verbose, new_interactive, new_max_size, new_ignore_pattern)
}
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Alias { action } => {
            ...
            match action {
                AliasAction::Save => {
                    // En una versión futura, esto se manejará con argumentos separados
                    println!("{} Para guardar un alias usa: alias save <nombre> <query> -- <parametros>", "💡".cyan());
                    println!("  Ejemplo: alias save prueba 'fn' -- --ext rs --path .");
                }
                AliasAction::List => {
                ...
                AliasAction::Remove => {
                    // En una versión futura se implementará con argumentos
                    println!("{} Para eliminar un alias usa: alias remove <nombre>", "💡".cyan());
                }
                AliasAction::Run => {
                    // En una versión futura se implementará con argumentos
                    println!("{} Para ejecutar un alias usa: search --alias <nombre>", "💡".cyan());
                }
            }
        }
        ...
        Commands::Index { path, ignore, force, ext, ignore_pattern } => {
            ...
            index_files(&path, ignore_dirs, ext_filter, ignore_pattern.as_deref(), force)?;
        }
                ...
                if let Some(alias_name) = alias {
                    ...
                    if let Some(entry) = config.aliases.get(&alias_name) {
                        ...
                        let (ext_vec, ignore_vec, v, i, max, pat) = apply_config_to_search(
                            new_ext, new_ignore, new_verbose, new_interactive, new_max_size, new_ignore_pattern, &config
                        );
                    ...
                    let (ext_vec, ignore_vec, v, i, max, pat) = apply_config_to_search(
                        ext, ignore, verbose, interactive, max_size, ignore_pattern, &config
                    );
                    (Some(q), ext_vec, ignore_vec, v, i, max, pat)
                } else {
